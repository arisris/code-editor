// original taken from: https://github.com/uiwjs/react-split/blob/master/src/index.tsx
...
export default class Split extends React.Component<SplitProps, SplitState> {
  ...
  onMouseDown(
    paneNumber: number,
    env: React.MouseEvent<HTMLDivElement, MouseEvent>
  ) {
  ...
  onDragging(env: Event) {
    ...
    if (mode === "horizontal") {
      ...
      this.preSize =
        (this.preSize / this.boxWidth >= 1 ? 1 : this.preSize / this.boxWidth) *
        100;
      this.nextSize =
    ...
    return (
      <div className={cls} {...other} ref={(node) => (this.warpper = node)}>
        {React.Children.map(child, (element: any, idx: number) => {
          ...
          const visiableBar =
            visiable === true ||
            (visiable && visiable.includes((idx + 1) as never)) ||
            false;
          const barProps = {
            className: [
              `${prefixCls}-bar`,
